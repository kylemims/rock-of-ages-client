  ...
  const displayRocks = () => {
    if (rocks && rocks.length) {
      return rocks.map((rock) => (
        <div
          key={`key-${rock.id}`}
          className="border p-5 border-solid hover:bg-fuchsia-500 hover:text-violet-500 rounded-md border-violet-900 mt-5 bg-slate-50">
          <div>
            {rock.name} ({rock.type?.label || "Unknown Type"})
          </div>
          <div>
            In the collection of {rock.user?.first_name || "Unknown"} {rock.user?.last_name || ""}
          </div>
          {showAll ? (
            ""
          ) : (
